...
public class RegistrarPersona extends JDialog {
	...
	public void PersonaHaciaBD(Persona persona) {
		
		String consulta = "INSERT INTO usuario(id_usuario, nombre, apellido, telefono, email, passwordU, Id_tipoU) VALUES (?,?,?,?,?,?,?)";
		try {

			con = ConexionSQL.getConexion();
			per = con.prepareStatement(consulta);
			per.setInt(1, Integer.parseInt(persona.getCedula()));
			per.setString(2, persona.getNombre());
			per.setString(3, persona.getNombre());
			per.setString(4, persona.getTelefono());
			per.setString(5,persona.getUsuarioString());
			per.setString(6, persona.getPassword());
